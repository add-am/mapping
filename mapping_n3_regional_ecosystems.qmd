...
#read in DES's data
test <- st_read("data/regional_ecosystems/Data_for_Adam.gdb")

test <- st_transform(test, proj_crs)

test <- test |> filter(str_detect(NAME, "BARRON"))

test <- test |> select(NAME) |> group_by(NAME) |> summarise(Shape = st_union(Shape)) |> st_zm()
...
barron_est <- test |> filter(str_detect(NAME, "AREA"))
...
barron_water <- test |> filter(str_detect(NAME, "CREEK"))
...
barron_rem <- st_intersection(barron_est, re_remnant) |> select(!NAME)
barron_pclear <- st_intersection(barron_est, re_p_clear) |> select(!NAME)
...
barron_rem_buff <- st_intersection(barron_water, re_remnant) |> select(!NAME)
barron_pclear_buff <- st_intersection(barron_water, re_p_clear) |> select(!NAME)

#create a list of files to loop over
file_list <- c("black_rem", "ross_rem", "black_pclear", "ross_pclear", "black_rem_buff", "ross_rem_buff",
               "black_pclear_buff", "ross_pclear_buff", "barron_rem", "barron_pclear", "barron_rem_buff", 
               "barron_pclear_buff")

for (i in 1:length(file_list)){
  
  if ("Shape" %in% colnames(get(file_list[i]))){
    
    assign(glue("{file_list[i]}"), (get(glue("{file_list[i]}")) |> rename(geometry = Shape)))
