...
It should be noted that many areas currently share more than one RE group. The code chunk below separates out areas that consist of more than one regional ecosystem group and assigns each an area based on its percentage weighting of the total area it represents.
``` {r}
#| label: explore data
#| output: false
...
Currently we are looking at two different RE data layers for each area: pre-clear, and the remnant vegetation as of 2019. Once the calculations and summaries above are done, the results for each RE layer for an area can be combined into one table. We can then compare these two layers and make comments on the change in cover over time.

Below we combine tables that refer to the same area.
...
However for many of the areas, the number of RE groups within is extensive (50+), therefore we combine the RE groups into broader categories, specifically:
...
For all the individual RE groups, as well as the summaries we create above, we then calculate the area change and percentage change.
...
Now all calculations are done we can save the output, the full tables for each area are combined into one multi-paged excel workbook, while the main summary groups calculated above are saved into another, simpler csv.
