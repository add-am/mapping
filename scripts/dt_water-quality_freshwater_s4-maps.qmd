...
year_folder <- glue("{here()}/outputs/dt_water-quality_freshwater_s4-maps/{current_fyear-1}-{current_fyear}_maps/")
...
nat <- rbind(st_transform(nat$osm_polygons, crs = proj_crs) |> select(name) |> filter(!is.na(name)),
             st_transform(nat$osm_multipolygons, crs = proj_crs) |> select(name) |> filter(!is.na(name)))
...
for (i in target_vect){
  ...
  #save map
  tmap_save(map, glue("{save_path}/{i}_Freshwater.png"), insets_tm = inset_map, insets_vp = vp)
...
for (i in target_vect){
  ...
  #save map
  tmap_save(map, glue("{save_path}/{i}_de-identified_Freshwater.png"), insets_tm = inset_map, insets_vp = vp)
