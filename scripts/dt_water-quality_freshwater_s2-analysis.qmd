...
description: "Script 2 in a series of script designed to analyse, score, and present freshwater water quality in the Dry Tropics region. The output of this is used in the Dry Tropics Technical Report."
author: "Adam Shand"
format: html
params:
...
The purpose of this script is to perform the main data analysis, as well as calculate final results for water quality data in the Dry Tropics technical report. Key steps include:
...
#set a path directly to the data 
...
year_folder <- glue("{here()}/outputs/dt_water-quality_freshwater_s2-analysis/{current_fyear-1}-{current_fyear}_analysis/")
...
freshwater_wq_cy <- freshwater_wq_cy |> 
...
fw_wq_monthly <- freshwater_wq_cy |> 
...
#stylize table
freshwater_summary_stats <- fw_wq_annual |> 
...
#save data
cond_form_wq_summary_stats(freshwater_summary_stats, glue("{save_path}/summary_statistics"), cols = c(5:6))
...
summary_table_present <- reactable(freshwater_summary_stats)
...
fw_wq_annual_wide <- fw_wq_annual |> 
...
#as above
basin_scores <- fw_wq_all |> 
  group_by(Region, Basin) |> 
  summarise(across(c(DIN, FRP, High_DO, Low_DO, Turbidity, TP, Nutrients, Phys_Chem, Overall_WQ), mean, na.rm = T))

#combine each of the datasets
fw_wq_all_scores <- bind_rows(fw_wq_all, sub_basin_scores, basin_scores) 
...
#save data
cond_form_rc_grades(sub_basin_scores_save, glue("{save_path}/fw_appendix_sub_basin_scores"), cols = c(2:4), method = "numeric")
...
fw_wq_sub_basin_weighted <- fw_wq_all_scores |> 
...
fw_wq_basin_weighted <- fw_wq_sub_basin_weighted |> 
...
fw_wq_all_weighted <- rbind(fw_wq_sub_basin_weighted, fw_wq_basin_weighted) |> 
  unique() |> 
...
fw_wq_all_scores_grades_raw <- fw_wq_all_weighted |> 
  mutate(across(9:length(fw_wq_all_weighted), 
...
fw_wq_all_scores_grades_floor <- fw_wq_all_scores_grades_raw |> 
...
write_csv(fw_wq_all_scores_grades_raw, glue("{save_path}/fw_wq_all_scores_grades_raw.csv"))

#save entire rounded dataset
write_csv(fw_wq_all_scores_grades_floor, glue("{save_path}/fw_wq_all_scores_grades_floor.csv"))

#do some yuck rearranging of data to make results align with the desired output tables.
nut_phys_chem_prep <- fw_wq_all_scores_grades_floor |> 
...
#save data
cond_form_rc_grades(nutrients_table, glue("{save_path}/fw_nutrients"), cols = c(4:10), method = "Numeric")
...
#save data
cond_form_rc_grades(phys_chem_table, glue("{save_path}/fw_phys_chem"), cols = c(4:11), method = "Numeric")
