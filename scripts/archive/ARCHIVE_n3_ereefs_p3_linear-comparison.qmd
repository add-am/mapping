...
This script is a supplementary script written to summarize the comparisons of ereefs model data against real-world logger data. This script compares data from;
 ...
 - Transformed model data:
        - obtained using terra and aims packages
 - Curvilinear model data:
        - obtained using the aims package only
        - obtained using the stars package
        
And compares data at times scales of:

 - daily from 2023-02-05 to 2023-02-28
 - daily from 2020 to 2022
 - hourly for a random day
 - hourly for all days from 2020 to 2022
 - midday only from 2020 to 2022
...
save_outputs <- here("outputs/n3_ereefs_p3_linear-comparison/")

#create a second path that will be used to save the datasets that we download
save_data <- here("data//n3_ereefs_p3_linear-comparison/")
...
First up is the MMP and IMOS loggers, we will get daily loggers for Pandora, Pelorus, and the IMOS ANMN stations, and hourly loggers only for Pelorus.
...
if (file.exists(glue("{save_data}/all_loggers.csv"))){#if data exists, read it in
  ...
  #read loggers
  pandora_daily_logger <- read.csv(glue("{save_data}/pandora_daily_logger_clean.csv")) |> 
  ...
  #read loggers
  pelorus_daily_logger <- read.csv(glue("{save_data}/pelorus_daily_logger_clean.csv")) |> 
  ...
  #read loggers
  imos_logger <- read.csv(glue("{save_data}/imos_anmn_logger_clean.csv")) |> 
    mutate(ID = "IMOS Logger",
           Freq = "Daily",
           Type = "Point",
           Hour = NA, 
           Depth = DEPTH) |> 
    mutate(CHLF = as.double(CHLF),
           CHLU = as.double(CHLU),
           CPHL = as.double(CPHL)) |> 
    rowwise() |> 
    mutate(Chla = mean(c(CHLF, CHLU, CPHL), na.rm = T)) |> 
    separate_wider_delim(TIME, "T", names = c("Date", NA)) |> 
    select(ID, Date, Hour, Freq, Type, CHLF, CHLU, CPHL, Chla)
    group_by(ID, Date, Hour, Freq, Type) |> 
    summarise(Chla = mean(Chla, na.rm = T)) |> 
    ungroup()

  #load in hourly data and do some trickery to get the 00:00:00 hour to play nicely
  pelorus_hourly_logger <- read.csv(glue("{save_data}/pelorus_hourly_logger_clean.csv")) |>
