...
for (i in 1:length(names)){
  ...
  n3_dem_base_map(dem = gbr, region = target, zscale = 30, sealevel = 0, highlight = T, rivers = T, preload = T)
...
A spicy extra that can be created is an STL file. STL files are used by 3D printers to create an object.
Below is all the code needed to complete this extra step.

``` {r}
#| label: create STL file

#pull up the plot
plot_3d(base_map_DT, dem_matrix_DT, zscale = 30, soliddepth = min(dem_matrix_DT)-800,
        water = F, background = "white", shadowcolor = "grey50", 
        shadowdepth = min(dem_matrix_DT)-1200, theta = 90, phi = 35, zoom = 0.47, fov = 16, 
        windowsize = c(50, 50, 1920, 1080))

#run the function
save_3dprint("tester3.stl", maxwidth = 200, unit = "mm")
