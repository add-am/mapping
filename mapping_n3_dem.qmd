...
#base_map, and a height matrix called dem_matrix
n3_dem_base_map(dem = gbr, region = region)
...
Additional customization can now be done to the 2D base map, including adding place 
names and rivers. The 2D map can then be remapped into 3D and saved with the new
features.

``` {r}
#| label: Get list of available places/towns/etc.
...
``` {r}
#| label: Get list of available rivers in region

#convert coords, set crs, and make into an extent object
location_extent <- extent(region)

#using the bbox from earlier
waterway <- opq(osm_bbox, timeout = 100) %>% 
  add_osm_feature("waterway") %>% 
  osmdata_sf()

#transform and filter data to only get line data
waterway <- st_transform(waterway$osm_lines, crs = crs(proj_crs))

#filter to remove columns and NAs
waterway <- waterway %>% 
  select(name, alt_name, intermittent, note, tunnel, waterway, geometry) %>% 
  filter(!is.na(name) & !is.na(waterway))

#get a list of unique waterway types
way_type <- unique(waterway$waterway)

river_overlay1 <- generate_line_overlay(waterway, 
...
test <- base_map %>% add_overlay(river_overlay1, alphalayer = 1)

plot_3d(base_map, dem_matrix, zscale = 20, soliddepth = -300,
