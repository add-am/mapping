...
Then we need to overlay the marine divisions that fall within our rough outline and group up all the
geometries that share the same properties. We also need to combine the offshore marine zone into one
polygon.
...
As the land and marine shapefiles were developed separately, they need to be combined. The below
code chunk does this combination.
...
The painful part of this process is that these divisions were created based on DES mapping 
requirements. Therefore the Black and Ross Basin outlines are slightly different from the Black and 
Ross Basin outlines used by the more widely accepted Drainage_basins shapefiles. The below code 
rectifies the biggest disparity between the basin outlines (the Alligator Ck/Cape Cleveland corner 
of the Ross Basin).
...
#save the file
st_write(tsv_wev_wt_wb, dsn = glue("{save_path}Dry-Tropics-Basins-Detailed.shp"),
         delete_layer = T)
